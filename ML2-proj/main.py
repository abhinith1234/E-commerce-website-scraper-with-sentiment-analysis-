...
def my_form_post():
    ...
    if len(search_quereies) >= 1:
        ...
        cursor.execute("SELECT * FROM emp_mgmt.flipkart")
        my_results = cursor.fetchall()

        for result in my_results:
            flipkart_upload.append(result)

        cursor.execute("SElECT * FROM emp_mgmt.amazon")
        my_results_2 = cursor.fetchall()

        for result in my_results_2:
            amazon_upload.append(result)

        cursor.execute("SELECT * FROM emp_mgmt.snapdeal")
